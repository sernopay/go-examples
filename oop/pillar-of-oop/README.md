...
## Inheritance
Inheritance is the process of creating a new class from an existing class. The new class inherits the properties and behaviors of the existing class. In Go, we can achieve inheritance by embedding structs. When we embed a struct in another struct, the embedded struct's fields and methods become part of the outer struct. \
...
we will create a "constructor" function that will initialize the concrete type and the base struct at the same time.
...
After making the struct unexported, we can not be able to intantiate the struct directly from outsite the package.
...
In the code above, we have created functions called `NewWordPrinter` and `NewPDFPrinter` that return a `Printer` interface. Since both `wordPrinter` and `pdfPrinter` implement the `Printer` interface, we can use them interchangeably. This is an example of polymophism in Go.

## Run the Code
To run the code, we need to creat a main function that will create instances of the concrete types and call their methods.
```go
func main() {
